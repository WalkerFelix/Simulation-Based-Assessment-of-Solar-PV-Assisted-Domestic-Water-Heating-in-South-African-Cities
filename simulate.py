...
def simulate_household_efficient(
    ...
    try:
        ...
        idx = irr_df.index
        # Get setpoints from config
        setpoint_solar = float(TANK_PARAMS['setpoint_solar'])
        setpoint_grid = float(TANK_PARAMS['setpoint_grid'])
        ...
        for season, ser in seasonal_Lpm.items():
            # Use solar setpoint as energy reference
            k5, L5 = _to_5min_energy_and_litres(ser, setpoint_solar, inlet_temp)
        ...
        tank.initialize(setpoint_grid) # Start at grid setpoint comfort level
        if not hasattr(tank, 'top_temp'):    tank.top_temp = setpoint_grid
        if not hasattr(tank, 'bottom_temp'): tank.bottom_temp = setpoint_grid
        
        # Baseline Tank (Grid only) (for unavoidable cold draws)
        tank_baseline = StratifiedTank(**TANK_PARAMS)
        tank_baseline.initialize(setpoint_grid)
        if not hasattr(tank_baseline, 'top_temp'):    tank_baseline.top_temp = setpoint_grid
        if not hasattr(tank_baseline, 'bottom_temp'): tank_baseline.bottom_temp = setpoint_grid
        ...
        for i, ts in enumerate(idx):
            ...
            # Determine dynamic setpoint based on PV availability
            ...
            # Switch between 65°C (Solar) and 55°C (Grid)
            active_sp = setpoint_solar if pv_allowed else setpoint_grid
            on_thr = active_sp - deadband
            off_thr = active_sp
            ...
            top_T, bot_T = tank.step(p_grid + p_pv_used, v_draw, t_amb, current_inlet_temp, active_sp)
            tank.top_temp = float(top_T)
            tank.bottom_temp = float(bot_T)

            # Baseline Tank Control (Grid Only - fixed at 55°C)
            base_on_thr = setpoint_grid - deadband
            base_off_thr = setpoint_grid
            if PERMANENT_LOAD_TEST:
                heater_on_baseline = True
            else:
                if not heater_on_baseline:
                    if (getattr(tank_baseline, 'bottom_temp', None) is not None and tank_baseline.bottom_temp <= base_on_thr) or (tank_baseline.top_temp <= base_on_thr):
                        heater_on_baseline = True
                if heater_on_baseline and tank_baseline.top_temp >= base_off_thr:
                    heater_on_baseline = False
            ...
            # Pass v_draw (Liters) instead of demand_kwh
            top_T_b, bot_T_b = tank_baseline.step(p_grid_baseline, v_draw, t_amb, current_inlet_temp, setpoint_grid)
        ...
        if DIAG_PRINTS:
            ...
            dT = max(1e-6, setpoint_grid - inlet_temp)
