...
def run_angle_sweep(
    ...
    for city in cities:
        ...
        for cat in SELECTED_PROFILES:
            ...
            if not all_rows:
                print(f"No results for {city} - {cat}")
                continue
            ...
            # Summary (mean across profiles for this specific category)
            ...
            # Save Excel per City and Category
            out_xlsx = os.path.join(ROOT_DIR, f"results_{city}_{cat}_angle_sweep.xlsx")
            ...
            try:
                ...
                def plot_heat(metric, fname_stub, cmap='viridis'):
                    ...
                    ax.set_title(f"{city} ({cat}): mean {metric} vs tilt×azimuth")
                    ax.grid(False)
                    out_png = os.path.join(ROOT_DIR, f"{fname_stub}_{city}_{cat}.png")
                ...
                print(f"    Plotting failed for {cat}: {e}")
...
def main():
    ...
    if RUN_SWEEP:
        ...
        run_angle_sweep(
            ...
            tilts=range(20, 56, 5),       # 0..60° step 5
            #azimuths=range(0, 361, 15),  # 0..360° step 15
            azimuths=[315, 330, 345, 0, 15, 30, 45, 60],
            n_jobs=-1
        )
        return
